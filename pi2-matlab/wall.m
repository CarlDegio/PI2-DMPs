...
for k=1:n_reps
  ...
  %mm = round(1/D(1).dt);
  %r(mm)  = r(mm) + 10000000000*((D(k).q(mm,1)-0.2)^2 + (D(k).q(mm,2)-0.2)^2);
  
  %obstacles
  for i=1:length(r)
      if D(k).q(i,1)<0.4 && D(k).q(i,2)>0.2
          r(i)=r(i)+10000000000*((D(k).q(i,2)-0.2)^2);
      end
      if D(k).q(i,1)>0 && D(k).q(i,2)<0.1
          r(i)=r(i)+10000000000*((D(k).q(i,2)-0.1)^2);
      end
  end
